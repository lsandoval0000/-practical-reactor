...
public class c2_TransformingSequence extends TransformingSequenceBase {
     ...
     * Casting using `map()` to cast is one way to do it, but there is more
     * convenient way.
     * Remove `map` operator and use more appropriate operator to cast sequence to
     * String.
     */
    @Test
    public void cast() {
    ...
    /***
     * Reduce the values from `numerical_service()` but emit each intermediary
     * number
    ...
    /**
     * A developer who wrote `numerical_service()` forgot that sequence should start
     * with zero, so you must prepend zero
     * to result sequence.
     *
