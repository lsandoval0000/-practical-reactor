...
public class c1_Introduction extends IntroductionBase {
     ...
     * Retrieve result from this Mono by blocking indefinitely or until a next
     * signal is received.
     */
    @Test
    ...
    /**
     * Retrieving result should last for a limited time amount of time, or you might
     * get in trouble.
     * Try retrieving result from service by blocking for maximum of 1 second or
    ...
    /**
     * Services are unpredictable, they might and might not return a result and no
     * one likes nasty NPE's.
    ...
    /**
     * Many services return more than one result and best services supports
     * streaming!
     * It's time to introduce Flux, an Asynchronous Sequence of 0-N Items.
     *
     * Service we are calling returns multiple items, but we are interested only in
     * the first one.
     * Retrieve first item from this Flux by blocking indefinitely until a first
     * item is received.
     */
    @Test
     ...
     * That really beats whole purpose of non-blocking and asynchronous library like
     * Reactor.
     * Blocking operators are usually used for testing or when there is no way
     * around, and
