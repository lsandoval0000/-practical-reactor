 ...
 * Reactor provides three batching strategies: grouping, windowing, and
 * buffering.
 *
 * Read first:
 *
...
public class c11_Batching extends BatchingBase {
    ...
    public void batch_writer() {
        Flux<Void> dataStream = dataStream().buffer(10).flatMap(buffer -> writeToDisk(buffer));
    ...
    /**
     * You are implementing a command gateway in CQRS based system. Each command
     * belongs to an aggregate and has `aggregateId`.
     ...
     * But commands that belong to different aggregates can and should be sent in
     * parallel.
     * Implement this behaviour by using `GroupedFlux`, and knowledge gained from
    ...
    public void command_gateway() {
        Flux<Void> processCommands = inputCommandStream()
                ...
                .flatMap(aCmd -> aCmd.concatMap(this::sendCommand));
    ...
    /**
     * You are implementing time-series database. You need to implement `sum over
     * time` operator. Calculate sum of all
