...
/**
 * Often we might require state when working with complex streams. Reactor
 * offers powerful context mechanism to share
 * state between operators, as we can't rely on thread-local variables, because
 * threads are not guaranteed to be the
...
public class c13_Context extends ContextBase {
     ...
     * Framework attaches a http correlation
     * id to the Reactor context. Your task is to extract the correlation id and
     * attach it to the message object.
     */
    public Mono<Message> messageHandler(String payload) {
        return Mono.deferContextual(ctx -> {
            String correlatedId = ctx.get(HTTP_CORRELATION_ID);
            return Mono.just(new Message(correlatedId, payload));
        });
    }
    ...
    /**
     * Following code counts how many times connection has been established. But
     ...
     * If the error occurs during a page retrieval, log the error message containing
     * page number that has an
