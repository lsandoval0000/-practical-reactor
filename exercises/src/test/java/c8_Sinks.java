...
/**
 * In Reactor a Sink allows safe manual triggering of signals. We will learn
 * more about multicasting and backpressure in
 * the next chapters.
 *
 * Read first:
 *
...
public class c8_Sinks extends SinksBase {
    ...
    /**
     * You need to execute operation that is submitted to legacy system which does
     * not support Reactive API. You want to
    ...
    /**
     * Similar to previous exercise, you need to execute operation that is submitted
     * to legacy system which does not
     * support Reactive API. This time you need to obtain result of
     * `get_measures_reading()` and emit it to subscriber.
     * If measurements arrive before subscribers subscribe to
     * `get_measures_readings()`, buffer them and emit them to
     ...
     * Subscribers should receive only the signals pushed through the sink after
    ...
    /**
     * By default, if all subscribers have cancelled (which basically means they
     * have all un-subscribed), sink clears
     * its internal buffer and stops accepting new subscribers. For this exercise,
     * you need to make sure that if all
     * subscribers have cancelled, the sink will still accept new subscribers.
    ...
    /**
     * If you look closely, in previous exercises third subscriber was able to
     * receive only two out of three
     * measurements. That's because used sink didn't remember history to re-emit all
     * elements to new subscriber.
     * Modify solution from `open_24_7` so third subscriber will receive all
     * measurements.
     */
    @Test
